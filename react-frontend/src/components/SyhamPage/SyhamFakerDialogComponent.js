...
import syhamFakerFactory from "./syhamFakerFactory";
...
const SyhamFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = syhamFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("syham").create(elem));
...
export default connect(null, mapDispatch)(SyhamFakerDialogComponent);
